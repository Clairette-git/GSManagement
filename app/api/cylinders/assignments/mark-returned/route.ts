...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      await db.query(
        "UPDATE cylinder_assignments SET is_returned = TRUE WHERE vehicle_assignment_id = ? AND cylinder_id = ?",
        [assignment_id, cylinder_id],
      )

      // Update the cylinder status to "returned"
      await db.query("UPDATE cylinders SET status = 'returned' WHERE id = ?", [cylinder_id])
      ...
      return NextResponse.json({
        message: "Cylinder marked as returned successfully",
      })
    } catch (error) {
