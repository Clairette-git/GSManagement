...
interface ClickableFilledCardProps {
  value: number
  period: string
}

export function ClickableFilledCard({ value, period }: ClickableFilledCardProps) {
  const router = useRouter()
  const [isHovered, setIsHovered] = useState(false)

  const handleClick = () => {
    router.push("/cylinders?status=filled")
  }
    ...
    >
      <CardContent className="p-6">
        <div className="flex flex-row items-center justify-between space-y-0 pb-2">
          <p className="text-sm font-medium text-teal-100">Filled Cylinders</p>
