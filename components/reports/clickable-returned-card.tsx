...
interface ClickableReturnedCardProps {
  value: number
  period: string
}

export function ClickableReturnedCard({ value, period }: ClickableReturnedCardProps) {
  const router = useRouter()
  const [isHovered, setIsHovered] = useState(false)

  const handleClick = () => {
    router.push("/cylinders?status=returned")
  }
    ...
    >
      <CardContent className="p-6">
        <div className="flex flex-row items-center justify-between space-y-0 pb-2">
          <p className="text-sm font-medium text-teal-100">Returned</p>
        ...
        <div className="flex items-center justify-between text-xs text-teal-100">
          ...
          <span className="text-teal-300">Click to view cylinders →</span>
        </div>
      </CardContent>
    </Card>
  )
